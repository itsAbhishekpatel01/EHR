    ...
    contract PermissionContract {
        ...
        function requestPermission(uint256 granterPublicKey) external {
            ...
            require(granterPublicKey!=(uint256(uint160(msg.sender))),"Requester & granter must not be same");
            // require(granterPublicKey != uint160(msg.sender), "Requester & granter must not be same");

            // Compute the permission key
            bytes32 permissionKey = keccak256(abi.encodePacked(address(uint160(granterPublicKey)), msg.sender));
        ...
        function approvePermission(uint256 requesterPublicKey) external {
            ...
            require(registry.checkUser(uint256(uint160(requesterPublicKey))), "Requester is not registered.");
            ...
            require(permission.requester == address(uint160(requesterPublicKey)), "Invalid requester or Invalid granter");
            // require(permission.granter == address(uint160(msg.sender)), "Only the granter can approve the request.");
